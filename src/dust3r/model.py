...
class LocalMemory(nn.Module):
    def __init__(
        ...
        # 这是一个更新的blocks 用改写新的 pose 到 local memory 里
        ...
        # 这是一个读取（readout） blocks，用来从memory里给当前帧读取 pose 初值
...
class ARCroco3DStereo(CroCoNet):
    ...
    def _decoder(self, f_state, pos_state, f_img, pos_img, f_pose, pos_pose):
        ...
        # 现在的图像侧token：[pose_token, patch_tokens...]
        
        final_output.append((f_state, f_img)) # 在 f_img 里加了 pose token 作为out的第2个元素
        for blk_state, blk_img in zip(self.dec_blocks_state, self.dec_blocks):
        # dec_blocks 用来更新当前帧输出的特征
        # dec_blocks_state 用来更新状态
        # final_output[-1] = (f_state_prev, f_img_prev)
            ...
            if (
                ...
                f_state, _ = checkpoint(
                    # croco 的实现习惯里 DecoderBlock(x, y, xpos, ypos) 的逻辑是：
                    # x 先 self-attention -> x 对 y cross attention -> x 过 MLP
                    # y 在整个过程中是不被改写的。
                    # 所以这里进去的 x = pos_state 被更新，y = pos_img 只是一个交互信息。
                    blk_state,
                    *final_output[-1][::+1],
                    pos_state,
                    pos_img,
    ...
    def _forward_decoder_step(
        ...
        if self.pose_head_flag:
            ...
            # 如果不是第一帧，那就可以从 pose memory 中检索一个 pose token 作为初值
        ...
        # 要把_decoder中拼进去的那个 pose token 找出来进行 memory 更新
        out_pose_feat_i = dec[-1][:, 0:1] 
        
        # 这个 pose memory 里把pose 和 appearance 都保存了，是一个关联记忆
